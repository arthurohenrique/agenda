    ...
    <main className="page-shell">
      <div className="page-container max-w-6xl">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <form className="relative"><label className="sr-only" htmlFor="customer-search">Buscar cliente</label><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" aria-hidden="true" size={17} /><input className="form-control pl-11 text-sm sm:w-80" defaultValue={query.q} id="customer-search" name="q" placeholder="Nome ou telefone" type="search" /></form>
        </div>

        <section className="premium-card mt-10 overflow-hidden">
          {filtered.length ? <div className="divide-y divide-zinc-100">{filtered.map((customer) => {
            const name = customer.display_name ?? customer.customers.full_name;
            return <article className="grid gap-4 p-5 transition hover:bg-[var(--background)] sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center sm:px-6 sm:py-5" key={customer.id}>
