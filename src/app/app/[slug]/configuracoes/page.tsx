...
import { ThemePalettePicker } from "@/components/admin/theme-palette-picker";
import { requireTenantAccess } from "@/features/tenants/access";
import { getPublicationChecklist, getTenantTheme } from "@/features/tenants/settings-queries";
import { getTenantPaletteId } from "@/lib/tenant-palettes";
...
export default async function SettingsPage({ params }: SettingsPageProps) {
  ...
  const [checklist, theme] = await Promise.all([
    getPublicationChecklist(tenant.id),
  ...
  return (
    <main className="page-shell">
      <div className="page-container max-w-5xl">
        ...
        <p className="page-description">Revise requisitos e controle publicação.</p>
        ...
        {canManage ? <ThemePalettePicker selectedPaletteId={getTenantPaletteId(theme)} slug={slug} /> : null}
      </div>
    </main>
  );
}
