  ...
  return (
    ...
    >
      <span className="min-w-0">
        {service.categoryName ? <span className="block text-sm font-semibold [overflow-wrap:anywhere]">{service.categoryName}</span> : null}
        <span className="mt-1 block text-lg font-semibold tracking-[-0.02em] [overflow-wrap:anywhere]">{service.name}</span>
    ...
    return (
      <section className="mx-auto max-w-2xl px-5 py-16 sm:py-24" aria-labelledby="booking-success-title">
        <div className="text-center">
          ...
          <h2 className="mt-3 text-4xl font-semibold tracking-[-0.05em]" id="booking-success-title" tabIndex={-1}>{pending ? "Já estamos cuidando disso." : "Seu horário está reservado."}</h2>
          <p className="mx-auto mt-4 max-w-lg leading-7 [overflow-wrap:anywhere]">
          ...
          <dl className="mt-10 grid gap-4 border-y border-[var(--booking-border)] py-6 text-left sm:grid-cols-2">
            <div className="min-w-0"><dt className="text-sm font-bold uppercase tracking-wide">Serviço</dt><dd className="mt-1 font-bold [overflow-wrap:anywhere]">{selectedService.name}</dd></div>
            <div className="min-w-0"><dt className="text-sm font-bold uppercase tracking-wide">Profissional</dt><dd className="mt-1 font-bold [overflow-wrap:anywhere]">{booking.staffName}</dd></div>
            <div><dt className="text-sm font-bold uppercase tracking-wide">Data</dt><dd className="mt-1 font-bold capitalize">{formatDateInTimezone(booking.startsAt, tenant.timezone)}</dd></div>
            <div><dt className="text-sm font-bold uppercase tracking-wide">Horário</dt><dd className="mt-1 font-bold">{formatTimeInTimezone(booking.startsAt, tenant.timezone)}</dd></div>
          </dl>
    ...
    <div className="mx-auto w-full max-w-3xl px-5 pb-24 pt-16 sm:px-8 sm:pt-24">
      {!selectedService ? (
        <section className="mx-auto max-w-2xl text-center" id="booking-service" aria-labelledby="services-title">
          ...
          <h2 className="mt-3 text-4xl font-semibold tracking-[-0.055em] sm:text-5xl" id="services-title" tabIndex={-1}>Como podemos ajudar?</h2>
          <p className="mt-4 text-base leading-7">Escolha um serviço. Os horários aparecem em seguida.</p>
        ...
        <div className="mx-auto max-w-3xl">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--booking-border)] pb-5">
            <div className="min-w-0">
              <p className="text-sm font-bold uppercase tracking-[0.14em]">Seu serviço</p>
              <p className="mt-1 truncate text-lg font-semibold tracking-[-0.025em]">{selectedService.name}</p>
            </div>
            <button className="min-h-11 rounded-full border border-[var(--booking-control-border)] bg-[var(--booking-surface)] px-4 text-sm font-medium hover:bg-[var(--booking-hover)]" onClick={clearService} type="button">Trocar</button>
          </div>
          ...
          <section className="scroll-mt-6 pt-10" id="booking-schedule" aria-labelledby="schedule-title">
            ...
            <h2 className="mt-3 text-4xl font-semibold tracking-[-0.055em] sm:text-5xl" id="schedule-title" tabIndex={-1}>Escolha um horário.</h2>
            <p className="mt-4 max-w-xl leading-7">Comece por um dos próximos dias. Você pode trocar a data quando quiser.</p>
            ...
            <div className="mt-8 flex gap-2 overflow-x-auto pb-2" aria-label="Datas sugeridas">
              {quickDates.map((date) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedDate === date.value ? "border-[var(--booking-primary)] bg-[var(--booking-primary)] text-white" : "border-[var(--booking-control-border)] bg-[var(--booking-surface)] hover:border-[var(--booking-control-border-strong)]",
                  )}
                  key={date.value}
              ...
              <label className="grid min-w-24 cursor-pointer place-items-center rounded-2xl border border-dashed border-[var(--booking-control-border)] bg-[var(--booking-surface)] px-3 text-center text-xs font-medium hover:border-[var(--booking-control-border-strong)]">
                Outra data
                <input aria-label="Escolher outra data" className="mt-1 max-w-20 bg-transparent text-center text-sm" min={initialDate} onChange={(event) => chooseDate(event.target.value)} type="date" value={selectedDate} />
              </label>
            </div>

            {selectedService.allowStaffSelection ? (
              <div className="mt-7">
                <button aria-controls={showStaffChoices ? "staff-choices" : undefined} aria-expanded={showStaffChoices} className="flex min-h-11 max-w-full min-w-0 items-center gap-2 text-left text-sm font-medium" onClick={() => setShowStaffChoices((visible) => !visible)} type="button">
                  <UserRound aria-hidden="true" size={17} />
                  <span className="min-w-0 [overflow-wrap:anywhere]">{staffId ? `Profissional: ${eligibleStaff.find((person) => person.id === staffId)?.name ?? "selecionado"}` : "Prefere escolher profissional?"}</span>
                  <ChevronDown aria-hidden="true" className={cn("transition", showStaffChoices && "rotate-180")} size={16} />
                </button>
                {showStaffChoices ? (
                  <div className="mt-3 flex gap-2 overflow-x-auto pb-2" id="staff-choices">
                    <button aria-pressed={!staffId} className={cn("min-h-11 shrink-0 rounded-xl border px-4 text-sm font-medium", !staffId ? "border-[var(--booking-primary)] bg-[var(--booking-primary)] text-white" : "border-[var(--booking-control-border)] bg-[var(--booking-surface)]")} onClick={() => chooseStaff(null)} type="button">Qualquer pessoa</button>
                    {eligibleStaff.map((person) => <button aria-pressed={staffId === person.id} className={cn("min-h-11 shrink-0 rounded-xl border px-4 text-sm font-medium", staffId === person.id ? "border-[var(--booking-primary)] bg-[var(--booking-primary)] text-white" : "border-[var(--booking-control-border)] bg-[var(--booking-surface)]")} key={person.id} onClick={() => chooseStaff(person.id)} type="button">{person.name}</button>)}
                  </div>
                ) : null}
              </div>
            ) : null}

            <div aria-busy={loadingSlots} className="mt-8">
              ...
              {loadingSlots ? (
                <div aria-hidden="true" className="grid grid-cols-3 gap-2 sm:grid-cols-4">{Array.from({ length: 8 }, (_, index) => <span className="h-14 animate-pulse rounded-2xl bg-[var(--booking-skeleton)]" key={index} />)}</div>
              ) : availabilityError ? (
                <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-medium text-red-800" role="alert">{availabilityError}</div>
              ) : slots.length ? (
                <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                  ...
                  {moreSlots.length ? <details className="col-span-3 sm:col-span-4"><summary className="mt-2 inline-flex min-h-11 cursor-pointer items-center text-sm font-medium">Ver mais {moreSlots.length} horários</summary><div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-4">{moreSlots.map((slot) => <SlotButton active={selectedSlot?.startAt === slot.startAt && selectedSlot.staffId === slot.staffId} key={`${slot.startAt}-${slot.staffId}`} onClick={() => chooseSlot(slot)} slot={slot} tenant={tenant} />)}</div></details> : null}
                </div>
              ) : (
                <div className="rounded-3xl border border-[var(--booking-border)] bg-[var(--booking-surface)] p-7 text-center"><CalendarDays className="mx-auto opacity-40" aria-hidden="true" size={26} /><p className="mt-3 font-semibold">Nenhum horário neste dia</p><p className="mt-1 text-sm">Tente uma das próximas datas.</p></div>
              )}
            </div>
          </section>
          ...
          {selectedSlot ? (
            <section className="scroll-mt-6 pt-14" id="booking-details" aria-labelledby="details-title">
              <div className="border-y border-[var(--booking-border)] py-10 sm:py-12">
                ...
                <h2 className="mt-3 text-3xl font-semibold tracking-[-0.05em]" id="details-title" tabIndex={-1}>Para quem é esse horário?</h2>
                <p className="mt-3 text-sm leading-6 [overflow-wrap:anywhere]">{formatDateInTimezone(selectedSlot.startAt, tenant.timezone)} às {formatTimeInTimezone(selectedSlot.startAt, tenant.timezone)} · {selectedSlot.staffName}</p>
                <form className="mt-7 grid gap-4 sm:grid-cols-2" onSubmit={form.handleSubmit(submitBooking)}>
                  <label className="grid gap-2 text-sm font-medium sm:col-span-2">
                    Nome completo
                    <input
                      aria-describedby={form.formState.errors.name ? "booking-name-error" : undefined}
                      aria-invalid={Boolean(form.formState.errors.name)}
                      autoComplete="name"
                      className="min-h-13 rounded-xl border border-[var(--booking-control-border)] bg-[var(--booking-background)] px-4 text-base font-normal focus:border-[var(--booking-accent)] focus:ring-4 focus:ring-blue-600/10"
                      {...form.register("name")}
                    />
                    {form.formState.errors.name ? <span className="text-sm text-red-700" id="booking-name-error" role="alert">{form.formState.errors.name.message}</span> : null}
                  </label>
                  <label className="grid gap-2 text-sm font-medium sm:col-span-2">
                    Telefone com DDD
                    <input
                      aria-describedby={form.formState.errors.phone ? "booking-phone-error" : undefined}
                      ...
                      {...form.register("phone")}
                    />
                    {form.formState.errors.phone ? <span className="text-sm text-red-700" id="booking-phone-error" role="alert">{form.formState.errors.phone.message}</span> : null}
                  </label>
                  <details className="sm:col-span-2">
                    <summary className="inline-flex min-h-11 cursor-pointer items-center text-sm font-medium">Adicionar e-mail ou observação (opcional)</summary>
                    <div className="mt-4 grid gap-4">
                      <label className="grid gap-2 text-sm font-medium">
                        E-mail
                        <input
                          aria-describedby={form.formState.errors.email ? "booking-email-error" : undefined}
                          aria-invalid={Boolean(form.formState.errors.email)}
                          autoComplete="email"
                          className="min-h-12 rounded-xl border border-[var(--booking-control-border)] bg-[var(--booking-background)] px-4 text-base font-normal"
                          type="email"
                          {...form.register("email")}
                        />
                        {form.formState.errors.email ? <span className="text-sm text-red-700" id="booking-email-error" role="alert">{form.formState.errors.email.message}</span> : null}
                      </label>
                  ...
                  <p className="min-w-0 text-sm leading-6 [overflow-wrap:anywhere] sm:col-span-2">Seus dados serão usados somente por {tenant.name} para administrar este atendimento.</p>
                </form>
              </div>
            </section>
          ) : null}
...
function SlotButton({ active, onClick, slot, tenant }: { active: boolean; onClick: () => void; slot: AvailableSlot; tenant: PublicTenant }) {
  return (
    <button
      ...
      aria-pressed={active}
      className={cn("min-h-14 rounded-2xl border px-3 text-sm font-semibold transition", active ? "border-[var(--booking-primary)] bg-[var(--booking-primary)] text-white" : "border-[var(--booking-control-border)] bg-[var(--booking-surface)] hover:border-[var(--booking-control-border-strong)]")}
      onClick={onClick}
      type="button"
    >
